...
void CPU_excecute(CPU &cpu, FullInstruction fullInstruction)
{
	...
	case SRL:
		...
		break;
	case SLA:
		CPU_SLA(cpu, fullInstruction.op1);
		break;
	case SRA:
		CPU_SRA(cpu, fullInstruction.op1);
		break;
	case RLC:
	...
	case RL:
		...
		break;
	case RRC:
		CPU_RRC(cpu, fullInstruction.op1);
		break;
	case RR:
...
void CPU_SLA(CPU& cpu, ArithmeticTarget target)
{
	...
	default:
		throw std::invalid_argument("CPU_SLA recieved invalid target!");
		return;
	}

	cpu.registers.f.carry = (*bytePointer >> 7) & 0x01;
...
void CPU_SRA(CPU& cpu, ArithmeticTarget target)
{
	...
	default:
		throw std::invalid_argument("CPU_SRA recieved invalid target!");
		return;
	}

	cpu.registers.f.carry = *bytePointer & 0x01;
	uint8_t bit7 = *bytePointer & 0x80;
...
void CPU_RLC(CPU& cpu, ArithmeticTarget target)
{
	...
	default:
		throw std::invalid_argument("CPU_RLC recieved invalid target!");
		return;
	}
...
void CPU_RRC(CPU& cpu, ArithmeticTarget target)
{
	cpu.pc += 2;
	uint8_t* bytePointer = nullptr;
	...
	default:
		throw std::invalid_argument("CPU_RRC recieved invalid target!");
		return;
	}

	uint8_t bit0 = *bytePointer & 0x01;
	...
	*bytePointer += (bit0 << 7);
	...
	cpu.registers.f.subtract = false;
	cpu.registers.f.half_carry = false;
	cpu.registers.f.carry = bit0;
}

void CPU_RR(CPU& cpu, ArithmeticTarget target)
{
	cpu.pc += 2;
	uint8_t* bytePointer = nullptr;
	...
	default:
		throw std::invalid_argument("CPU_RR recieved invalid target!");
		return;
	}
	...
	*bytePointer >>= 1;
	if (cpu.registers.f.carry) { *bytePointer += 0x80; }
